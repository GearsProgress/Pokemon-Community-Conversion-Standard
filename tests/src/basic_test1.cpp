...
TEST_CASE("Basic gen 3 test - check that pokemon data is parsed correctly", "[unit][gen3][pokemonparsing]")
{
    ...
    bulbasaur.loadData(g3_bulbasaur_data);

    // these things need to be called for the values of the data substructure to be readable
    // https://bulbapedia.bulbagarden.net/wiki/Pok%C3%A9mon_data_substructures_(Generation_III)
    bulbasaur.updateChecksum();
    bulbasaur.decryptSubstructures();
    bulbasaur.updateSubstructureShift();
